      ...
      <Row noGutters>
        <Col xs={{ span: 10, offset: 1 }} md={{ span: 6, offset: 3 }}>
          <p>
            ...
            game for participants of my research to play to ensure they were
            being exposed to a game with violent content.
          </p>
          <iframe
          ...
          />
          <p>
            The enemies in the game also use Unity’s navigation system to path
            find to predetermined locations. To do this a navigation mesh was
            created by the engine by calculating the geometry of the map. Empty
            objects that act as points for each of the enemies were then placed
            in the level and placed into an array for each of the enemy’s
            navigation script to know what waypoints to move to. The enemy
            movement code then moves to the first waypoint in the array until it
            has reached it then selects the next waypoint and keeps cycling
            through the array. The enemies are also told to pause once they have
            reach each waypoint and will also stop moving if the player is
            within range of them or if the player shoots at them. The enemies
            will also shoot back at the player using the same Recasting method
