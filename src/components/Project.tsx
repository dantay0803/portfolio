...
const Project = ({title, tools, description, image, reverse, ProjectDetails}: ProjectProps) => {
  const [isModalOpen, setIsModalOpen] = useState(false)

  return (
    <div onClick={() => setIsModalOpen(true)} className={`w-full flex flex-col gap-y-1 ${reverse ? 'justify-end text-right' : 'justify-start text-left'} hover:cursor-pointer`}>
