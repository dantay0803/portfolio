    ...
    <div className='w-full bg-secondary'>
      <div className='mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-5 lg:pb-10'>
        <div className='mx-auto max-w-2xl flex-shrink-0 md:mx-0 pb-5 pt-5 lg:pt-10'>
          <p className='text-accent-dark text-lg underline'>Things I've worked on solo and as part of a team</p>
          <h2 className='uppercase text-black text-6xl mb-8 font-semibold'>Projects</h2>
        </div>
        <div className='w-full flex flex-col gap-y-10'>
          {PROJECTS.map((project, index) => (<Project key={project.title} title={project.title} tools={project.tools} description={project.description} image={project.image} reverse={index % 2 !== 0} />))}
        </div>
      </div>
    </div>
  )
