    ...
    <div id="resume" className='w-full'>
      <div className='mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-5 lg:pb-10'>
        <div className='mx-auto max-w-2xl flex-shrink-0 md:mx-0 pb-5 pt-5 lg:pt-10'>
          ...
          <h2 className='uppercase text-black text-6xl mb-8 font-semibold'>Resume</h2>
        </div>
        <div className='flex flex-col justify-start items-start mx-auto lg:mx-0'>
          {WORK.map((work) => (<WorkExperience key={`${work.company}-experience`} company={work.company} link={work.link} date={work.date} role={work.role} description={work.description} />))}
          <Education />
          <Awards />
        </div>
      </div>
